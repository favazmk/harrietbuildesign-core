...
function Digit({ place, value, height, digitStyle, springOptions }: DigitProps) {
    let valueRoundedToPlace = Math.floor(value / place);
    let animatedValue = useSpring(valueRoundedToPlace, springOptions); // Pass spring config
...
export default function Counter({
    value,
    fontSize = 100, // Default changed to reasonable default if not passed, but will control via props
    padding = 0,
    places = [100, 10, 1],
    gap = 8,
    borderRadius = 4,
    ...
    return (
        <div style={{ ...defaultContainerStyle, ...containerStyle }}>
            <div style={{ ...defaultCounterStyle, ...counterStyle }}>
                {places.map((place) => (
                    <Digit key={place} place={place} value={value} height={height} digitStyle={digitStyle} springOptions={springOptions} />
                ))}
            </div>
